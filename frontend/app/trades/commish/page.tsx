...
import { isCommission } from "@/lib/auth";
...
async function summarize(tradeId: number) {
  ...
  const [players, prospects, picks] = await Promise.all([
    ...
    pkids.length ? prisma.draftPick.findMany({ where: { id: { in: pkids } }, select: { id: true, year: true, round: true } }) : [],
  ]);
  const pN = new Map(players.map((p) => [p.id, cleanName(p.name)]));
  const prN = new Map(prospects.map((p) => [p.id, cleanName(p.name)]));
  const pkN = new Map(picks.map((p) => [p.id, `${p.year} R${p.round}`]));
...
export default async function CommishTradesPage() {
  ...
  const trades = await prisma.trade.findMany({
    where: { status: { in: ["AWAITING_COMMISH", "MODIFY", "MODIFIED"] } },
  ...
  const rows = await Promise.all(trades.map(async (t) => ({ t, sum: await summarize(t.id) })));
