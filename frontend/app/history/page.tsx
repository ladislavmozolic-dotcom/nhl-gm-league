...
const AWARD_ORDER = ["Hart", "Art Ross", "Rocket Richard", "Norris", "Vezina", "Calder", "Selke", "Lady Byng", "Conn Smythe", "Jack Adams"];
...
export default async function HistoryPage() {
  const [records, awards, teams] = await Promise.all([
  ...
  if (records.length === 0) {
    return (
      <div className="space-y-6 py-2">
        <PageHeader title="League History" subtitle="Champions and award winners, archived season by season." />
        <ComingSoon title="No seasons archived yet" points={[
          ...
          "Each entry captures Stanley Cup & Calder Cup champions, President's Trophy, and the major award winners.",
          "Once archived, seasons appear here newest-first with full winners.",
        ]} />
      </div>
    );
  }
  ...
  // resolve player names/slugs referenced by awards
  ...
  const LeagueBlock = ({ season, league }: { season: string; league: string }) => {
    ...
    if (!rec) return null;
    const la = awards.filter((a) => a.season === season && a.league === league)
      .sort((a, b) => AWARD_ORDER.indexOf(a.category) - AWARD_ORDER.indexOf(b.category));
    ...
    return (
      <Card title={`${league} — ${cupName}`} accent={league === "AHL" ? "text-orange-400" : "text-blue-400"}>
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            {la.length ? la.map((a) => <AwardRow key={a.id} a={a} />) : <p className="text-sm text-slate-500">No awards recorded.</p>}
          </div>
        </div>
      </Card>
    );
