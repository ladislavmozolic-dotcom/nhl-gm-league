...
export default async function AllRostersPage({ searchParams }: { searchParams: Promise<{ team?: string }> }) {
  ...
  // Players parked off the active roster via Mark LTIR / Send to Prospects
  // (rosterType "PROSPECT"), split by the ltir flag — matches the two sections
  // on /teams/[slug]/prospects. Separate from the Prospect model's draft
  // prospects below.
  ...
  return (
    <div className="space-y-6 py-2">
      <PageHeader title="All Rosters" subtitle="Complete rosters, prospects, picks and conditions for every club."
        right={<Link href="/tools/all-goalies" className="text-sm px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-blue-600 text-slate-300 hover:text-white transition-colors whitespace-nowrap">🥅 All Goalies</Link>}
      />
      ...
      <div>
        ...
        <RosterTable title="Skaters" players={farmSkaters.map((p) => toRP(p, farmHasField))} hideAttrs={!loggedIn} />
        <RosterTable title="Goalies" players={farmGoalies.map((p) => toRP(p, farmHasField))} goalie hideAttrs={!loggedIn} />
      </div>

      {(reserve.length > 0 || prospectPool.length > 0) && (() => {
        ...
        return (
          <>
            {reserve.length > 0 && (
              <div>
                <SectionTitle accent="text-amber-300">Reserve List <span className="text-[10px] font-semibold uppercase tracking-wider text-amber-400/80">LTIR · off the cap</span></SectionTitle>
                <OffRosterTable rows={reserve} />
              </div>
            )}
            {prospectPool.length > 0 && (
              <div>
                <SectionTitle accent="text-blue-400">Prospect Pool <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">left the NHL — e.g. Europe</span></SectionTitle>
