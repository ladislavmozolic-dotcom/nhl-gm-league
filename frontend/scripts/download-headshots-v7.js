    ...
    client.get(url, {
      ...
      if (res.statusCode === 301 || res.statusCode === 302) {
        ...
        if (redirect) {
          ...
          httpGet(full, headers, retries).then(resolve).catch(reject);
          return;
        }
      }
      let data = "";
      ...
      if (retries > 0) {
        await sleep(3000);
        httpGet(url, headers, retries - 1).then(resolve).catch(reject);
      } else {
        reject(err);
      }
    }).on("timeout", async () => {
...
async function main() {
  ...
  for (const player of players) {
    ...
    // Dynamic delay: longer if we're getting rate limited
    const baseDelay = 2000;
    const failPenalty = Math.min(consecutiveFails * 1000, 10000);
    const jitter = Math.random() * 1000;
    const totalDelay = baseDelay + failPenalty + jitter;

    console.log(`    Waiting ${(totalDelay/1000).toFixed(1)}s...`);
