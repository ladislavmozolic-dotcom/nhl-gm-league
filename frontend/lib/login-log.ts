...
/** Record a successful GM sign-in with IP + a best-effort geolocation. Never throws. */
export async function recordLogin(teamId: number): Promise<void> {
  try {
    ...
    await prisma.loginLog.create({
      data: { teamId, ip, userAgent, country: geo.country ?? null, region: geo.regionName ?? null, city: geo.city ?? null, isp: geo.isp ?? null },
    });
  } catch { /* auditing must never block a login */ }
}
