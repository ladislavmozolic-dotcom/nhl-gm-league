...
export async function leagueDetailedFinance(season = REGULAR_SEASON): Promise<Map<number, DetailedClubFinance>> {
  const [fans, stars, teams, playoffSeries, playoffGames] = await Promise.all([
    leagueFanInterest(),
    allStarPowers(),
  ...
  for (const game of playoffGames) {
    ...
    if (!club) continue;
    const gate = playoffGameRevenue(game.round ?? 1, club.attendancePct, club.capacity);
  ...
  for (const club of working) {
    ...
    const playoffMerch = round > 0 ? Math.round(club.merchTotal * (playoffMerchBoost(round) - 1)) : 0;
    ...
    out.set(club.teamId, { revenue, expenses: club.expenses, salary: club.salary, net: revenue - club.expenses, revenueLines, expenseLines: club.expenseLines });
  }
  return out;
}
