...
// `value` is already a percentile (0-100) against other NHL players at the
// same position group — see lib/line-builder-server.ts profileOf — so the bar
// width IS the value, no separate ceiling/scale needed.
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
      ...
      <div className="space-y-1 mb-3">
        <div className="text-[10px] uppercase tracking-wide text-slate-500 mb-1" title={`Percentile vs. other NHL ${line.kind === "D" ? "defensemen" : "forwards"} — 100 means nobody at the position rates higher, 50 is dead average.`}>Profile</div>
        <Bar label="Playmaking" value={p.playmaking} />
        <Bar label="Shooting" value={p.shooting} />
        <Bar label="Skating" value={p.skating} />
    ...
    <div className="space-y-6">
      ...
      <details className="bg-slate-900/40 border border-slate-800 rounded-xl px-4 py-2 text-sm">
        ...
        <div className="mt-2 space-y-2 text-slate-400 text-[13px] leading-snug">
          ...
          <p><span className="font-semibold text-slate-300">♟️ Tactical Fit</span> — how well the line is <em>constructed</em> (no games needed): role diversity (a playmaker + sniper + grinder, or an offensive D + a shutdown D, scores high; three snipers or two identical D score low) × position &amp; handedness correctness (centre at C, wingers on their side; LD shoots left, RD shoots right) × how well these specific players suit your team&apos;s chosen system in Tactics (a Rush system wants finishing/passing/speed on the line, a Cycle system wants passing/strength — neutral if you&apos;re running Balanced). It updates the moment you change players or your team system.</p>
        </div>
      </details>
      <div>
