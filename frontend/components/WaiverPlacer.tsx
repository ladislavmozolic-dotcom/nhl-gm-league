...
import { WAIVER_CAP_HIT_LIMIT } from "@/lib/roster-rules";
...
export default function WaiverPlacer({ teamId, players }: { teamId: number; players: MyPlayer[] }) {
  ...
  return (
    <Card title="Place a player on waivers" accent="text-sky-400">
      <p className="text-xs text-slate-500 mb-2">A player must clear waivers before he can be sent to the AHL. He stays on your roster and cap until the window closes tomorrow; any club can claim him first (worst team in the standings gets priority). An <b>NMC blocks waivers</b>, and a cap hit of {money(WAIVER_CAP_HIT_LIMIT)}+ is too valuable to waive.</p>
          ...
          return (
            <div key={p.id} className="py-2 flex items-center gap-3">
              ...
              {p.onWaivers ? <span className="text-xs text-sky-400">on waivers</span>
                ...
                : tooExpensive ? <span className="text-xs text-slate-600" title="Cap hit too high to waive">too expensive</span>
