...
export type StandingRow = {
  ...
  points: number; rw: number; row: number; sow: number; sol: number;
  gf: number; ga: number; diff: number; logoUrl: string | null; slug: string | null;
};

type ColDef = { key: keyof StandingRow | "team" | "so"; label: string; title?: string; align: "left" | "center"; num?: boolean };
const COLS: ColDef[] = [
  ...
  { key: "rw", label: "RW", title: "Regulation Wins", align: "center", num: true },
  { key: "row", label: "ROW", title: "Regulation + OT Wins", align: "center", num: true },
  { key: "so", label: "SO", title: "Shootout Record (W-L)", align: "center" },
...
export default function StandingsTable({ rows }: { rows: StandingRow[] }) {
  ...
  const sorted = useMemo(() => {
    ...
    const arr = [...rows].sort((a, b) => {
      if (sort === "so") {
        const va = a.sow * 100 - a.sol;
        const vb = b.sow * 100 - b.sol;
          ...
          <thead>
            <tr className="border-b border-slate-800 bg-slate-800/30 text-slate-500 text-xs uppercase tracking-wider select-none">
              ...
              {COLS.map((c) => {
                ...
                return (
                  <th key={c.key} onClick={() => click(c)} title={c.title}
                    className={`px-3 py-3 font-medium whitespace-nowrap ${c.align === "left" ? "text-left px-4" : "text-center"} ${c.key !== "team" ? "cursor-pointer hover:text-slate-200" : ""} ${active ? "text-blue-400" : ""}`}>
          ...
          <tbody>
            {sorted.map((t, i) => (
              <tr key={t.teamId} className="border-b border-slate-800/40 hover:bg-slate-800/30 transition-colors last:border-0">
                <td className="px-4 py-2.5 font-bold text-slate-500 whitespace-nowrap">{i + 1}</td>
                ...
                <td className="px-3 py-2.5 text-center text-slate-300 tabular-nums">{t.rw}</td>
                <td className="px-3 py-2.5 text-center text-slate-300 tabular-nums">{t.row}</td>
                <td className="px-3 py-2.5 text-center text-slate-400 tabular-nums">{t.sow}-{t.sol}</td>
